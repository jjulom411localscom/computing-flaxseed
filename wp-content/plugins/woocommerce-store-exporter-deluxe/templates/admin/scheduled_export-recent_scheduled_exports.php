...
<?php if( !empty( $recent_exports ) ) { ?>
	...
	<?php } ?>
			...
			<p style="text-align:right;">
				<a href="<?php echo esc_url( add_query_arg( array( 'action' => 'nuke_recent_scheduled_exports', '_wpnonce' => wp_create_nonce( 'woo_ce_nuke_recent_scheduled_exports' ) ) ) ); ?>" class="button action confirm-button" style="float:left;" data-confirm="<?php _e( 'This will permanently clear the contents of the Recent Scheduled Exports list. Are you sure you want to proceed?', 'woocommerce-exporter' ); ?>"><?php _e( 'Delete All', 'woocommerce-exporter' ); ?></a>
				<?php printf( __( '%d items', 'woocommerce-exporter' ), $size ); ?>
			</p>
<?php } else { ?>
