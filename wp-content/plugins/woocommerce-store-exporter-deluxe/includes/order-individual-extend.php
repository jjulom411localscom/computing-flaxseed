...
function woo_ce_extend_order_items_individual( $order, $order_item ) {
	...
	if( ( woo_ce_detect_export_plugin( 'gravity_forms' ) && woo_ce_detect_export_plugin( 'woocommerce_gravity_forms' ) ) && $order->order_items ) {
		...
		if( WOO_CD_DEBUG )
			error_log( 'woo_ce_get_gravity_form_fields(): ' . ( time() - $export->start_time ) );
	}
	...
	if( woo_ce_detect_export_plugin( 'woocommerce_bookings' ) ) {
		...
		unset( $booking_type );
		if( WOO_CD_DEBUG )
			error_log( 'woo_ce_get_order_assoc_booking_id(): ' . ( time() - $export->start_time ) );
	}
	...
	if( woo_ce_detect_export_plugin( 'extra_product_options' ) ) {
		...
		if( WOO_CD_DEBUG )
			error_log( 'woo_ce_get_extra_product_option_fields(): ' . ( time() - $export->start_time ) );
	}
	...
	if( !empty( $tax_rates ) ) {
		...
		if( WOO_CD_DEBUG )
			error_log( 'woo_ce_get_order_tax_rates(): ' . ( time() - $export->start_time ) );
	}
	unset( $tax_rates, $tax_rate );

	// Attributes
	if( $attributes = woo_ce_get_product_attributes( 'attribute_name' ) ) {
		...
		if( WOO_CD_DEBUG )
			error_log( 'woo_ce_get_product_attributes(): ' . ( time() - $export->start_time ) );
	}
	...
	if( class_exists( 'WC_Ship_Multiple' ) ) {
		...
		if( WOO_CD_DEBUG )
			error_log( 'shipping_packages: ' . ( time() - $export->start_time ) );
	}
	...
	if( !empty( $custom_order_items ) ) {
		foreach( $custom_order_items as $custom_order_item ) {
			if( !empty( $custom_order_item ) )
				$order->{sprintf( 'order_items_%s', $custom_order_item )} = $order_item->{$custom_order_item};
		}
	}
	...
	if( !empty( $custom_order_products ) ) {
		foreach( $custom_order_products as $custom_order_product ) {
			if( !empty( $custom_order_product ) )
				$order->{sprintf( 'order_items_%s', sanitize_key( $custom_order_product ) )} = $order_item->{sanitize_key( $custom_order_product )};
		}
	}
	...
	if( !empty( $custom_products ) ) {
		foreach( $custom_products as $custom_product ) {
			if( !empty( $custom_product ) )
				$order->{sprintf( 'order_items_%s', sanitize_key( $custom_product ) )} = $order_item->{sanitize_key( $custom_product )};
		}
	}
