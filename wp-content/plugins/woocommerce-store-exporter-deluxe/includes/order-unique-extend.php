		...
		foreach( $custom_order_products as $custom_order_product ) {
			if( !empty( $custom_order_product ) ) {
				if( isset( $order_item->{sanitize_key( $custom_order_product )} ) )
					$order->{sprintf( 'order_item_%d_%s', $i, sanitize_key( $custom_order_product ) )} = $order_item->{sanitize_key( $custom_order_product )};
			}
		}
	}
		...
		foreach( $custom_products as $custom_product ) {
			if( !empty( $custom_product ) ) {
				if( isset( $order_item->{sanitize_key( $custom_product )} ) )
					$order->{sprintf( 'order_item_%d_%s', $i, sanitize_key( $custom_product ) )} = $order_item->{sanitize_key( $custom_product )};
			}
		}
	}
