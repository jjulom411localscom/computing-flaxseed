...
function woo_ce_extend_order_items_combined( $order ) {
			...
			foreach( $product_addons as $product_addon ) {
				foreach( $order->order_items as $order_item ) {
					if( isset( $order_item->product_addons[$product_addon->post_name] ) )
						$order->{sprintf( 'order_items_product_addon_%s', $product_addon->post_name )} .= $order_item->product_addons[$product_addon->post_name] . $export->category_separator;
				}
				if( isset( $order->{sprintf( 'order_items_product_addon_%s', $product_addon->post_name )} ) )
					$order->{sprintf( 'order_items_product_addon_%s', $product_addon->post_name )} = substr( $order->{sprintf( 'order_items_product_addon_%s', $product_addon->post_name )}, 0, -1 );
			}
			unset( $product_addons, $product_addon );
		}
		if( WOO_CD_DEBUG )
			error_log( 'woo_ce_get_product_addons(): ' . ( time() - $export->start_time ) );
	}
	...
	if( woo_ce_detect_export_plugin( 'woocommerce_bookings' ) && $order->order_items ) {
		...
		foreach( $order->order_items as $order_item ) {
			...
			if( !empty( $booking_id ) ) {
				// @mod - Are we double querying here?
		...
		foreach( $order->order_items as $order_item ) {
			foreach( $attributes as $attribute ) {
				if( isset( $order_item->{sprintf( 'attribute_%s', sanitize_key( $attribute->attribute_name ) )} ) )
					$order->{sprintf( 'order_items_attribute_%s', sanitize_key( $attribute->attribute_name ) )} .= $order_item->{sprintf( 'attribute_%s', sanitize_key( $attribute->attribute_name ) )} . $export->category_separator;
				if( isset( $order_item->{sprintf( 'product_attribute_%s', sanitize_key( $attribute->attribute_name ) )} ) )
					$order->{sprintf( 'order_items_product_attribute_%s', sanitize_key( $attribute->attribute_name ) )} .= $order_item->{sprintf( 'product_attribute_%s', sanitize_key( $attribute->attribute_name ) )} . $export->category_separator;
			}
		}
		foreach( $attributes as $attribute ) {
		...
		foreach( $order->order_items as $order_item ) {
			foreach( $custom_order_items as $custom_order_item ) {
				if( !empty( $custom_order_item ) )
					$order->{sprintf( 'order_items_%s', $custom_order_item )} .= $order_item->{$custom_order_item} . $export->category_separator;
			}
		}
		foreach( $custom_order_items as $custom_order_item ) {
		...
		foreach( $order->order_items as $order_item ) {
			foreach( $custom_order_products as $custom_order_product ) {
				if( !empty( $custom_order_product ) )
					$order->{sprintf( 'order_items_%s', sanitize_key( $custom_order_product ) )} .= $order_item->{sanitize_key( $custom_order_product )} . $export->category_separator;
			}
		}
		foreach( $custom_order_products as $custom_order_product ) {
