...
class AffiliateWP_PayPal_API {
	...
	public function send_payment( $args = array() ) {
		...
		$request = wp_remote_post( 'https://api.' . $this->sandbox . 'paypal.com/v1/payments/payouts?sync_mode=true', array(
			'headers'    => array(
	...
	public function send_bulk_payment( $payouts = array() ) {
		...
		$request = wp_remote_post( 'https://api.' . $this->sandbox . 'paypal.com/v1/payments/payouts', array(
			'headers'     => array(
				'Content-Type'  => 'application/json',
				'Authorization' => 'Bearer ' . $token->access_token,
				'PayPal-Partner-Attribution-Id' => 'EasyDigitalDownloads_SP',
