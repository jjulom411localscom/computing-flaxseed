...
-Null24.Net	 *
	...
	public static function create_pending_subscription_for_order( $order, $product_id, $args = array() ) {
		...
		$subscription->add_order_note( __( 'Pending subscription created.-Null24', 'woocommerce-subscriptions' ) );
	...
	public static function touch_time( $args = array() ) {
		...
		for ( $i = 1; $i < 13; $i = $i + 1 ) {
			...
			$month_input .= '>' . sprintf( _x( '%1$s-%2$s', 'used in a select box-Null24', 'woocommerce-subscriptions' ), $month_numeral, $wp_locale->get_month_abbrev( $wp_locale->get_month( $i ) ) ) . "</option>\n";
		}
		$month_input .= '</select>';
