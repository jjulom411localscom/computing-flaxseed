...
class WC_Zapier_Admin_Feed_UI {
	...
	public function __construct() {
		...
		$this->meta_fields = array(
			array(
				'type' => 'title',
				'desc' => __( '<p>To configure your Zapier Feed, complete the information below.</p><p>' . sprintf( __( 'Note: The setup process is quite involved, <a href="%s" target="_blank" title="(Opens in a new window)">so we strongly recommend reading the documentation</a>.', 'wc_zapier' ), WC_Zapier::documentation_url ) . ' </p>', 'wc_zapier' ),
	...
	public function save_post( $post_id, $post ) {
		...
		if ( is_array( $validation_results ) ) {
			...
			if ( !empty( $validation_results['errors'] ) ) {
				...
				$messages[$post->ID]['errors'][] = _n( 'This Zapier Feed cannot be activated until this issue is resolved.', 'This Zapier Feed cannot be made active until these issues are resolved.', sizeof( $messages[$post->ID]['errors'] ), 'wc_zapier' );
			}
			update_option( 'wc_zapier_feed_messages', $messages );
		}
					...
					switch ( $field['id'] ) {
						case "{$this->prefix}trigger":
							...
							if ( !is_null( $feed->{$field['maps_to']}() ) ) // Just in case the trigger key in the database no longer exists
								echo $feed->{$field['maps_to']}()->get_trigger_title();
							break;
						default:
							echo $feed->{$field['maps_to']}();
					}
				}
			}
		}
