...
<table class="order-details">
	<thead>
		<tr>
			...
			<th class="unit-price"><?php _e('Unit Price', 'wpo_wcpdf'); ?></th>
	...
	<tbody>
		...
		<tr class="<?php echo apply_filters( 'wpo_wcpdf_item_row_class', $item_id, $wpo_wcpdf->export->template_type, $wpo_wcpdf->export->order, $item_id ); ?>">
			...
			<td class="unit-price"><?php echo $item['ex_single_price']; ?></td>
	...
	<tfoot>
		<tr class="no-borders">
			...
			<td class="no-borders" colspan="1"></td>
			<td class="no-borders" colspan="2">
				<table class="totals">
					<tfoot>
						<tr class="<?php echo $key; ?>">
							<td class="no-borders"></td>
							<th class="description"><?php echo 'Subtotal'; ?></th>
							<td class="price"><span class="totals-price"><?php echo $item['line_subtotal']; ?></span></td>
						</tr>
						<tr class="<?php echo $key; ?>">
							<td class="no-borders"></td>
							<th class="description"><?php echo 'Sales Tax'; ?></th>
							<td class="price"><span class="totals-price"><?php echo $item['line_subtotal_tax']; ?></span></td>
						</tr>
						<?php $record_one = 'Shipping';
								$record_two = 'Total';
						 foreach( $wpo_wcpdf->get_woocommerce_totals() as $key => $total ) : ?>
						 <?php if ($total['label'] == $record_one || $total['label'] == $record_two){?>
						<tr class="<?php echo $key; ?> row-totals">
							<td class="no-borders"></td>
							<th class="description"><?php if( $total['label'] == 'Shipping' ){ echo 'Shipping';}elseif($total['label'] == 'Total'){echo 'Total'; }else{} ?></th>
